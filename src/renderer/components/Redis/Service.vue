...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState('app', {
        serverRunning: state => state.stat.redis
      }),
      ...mapState('preference', {
    ...
    methods: {
      serviceDo (flag) {
        ...
        this.$store.dispatch('redis/start', { type: 'redis-service', v: this.version })
        let arg = ''
        switch (flag) {
          case 'stop':
            arg = 'stopService'
        ...
        this.$electron.ipcRenderer.send('command', 'redis', arg, this.version)
      }
    },
    created: function () {
      if (!this.$store.state.redis.taskRunning) {
        this.$store.dispatch('redis/cleanLog')
      }
    }
  }
</script>
