...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState('app', {
        serverRunning: state => state.stat.apache
      }),
      ...mapState('preference', {
    ...
    methods: {
      serviceDo (flag) {
        ...
        this.$store.dispatch('apache/start', { v: this.version })
        let arg = ''
        switch (flag) {
          case 'stop':
            arg = 'stopService'
        ...
        this.$electron.ipcRenderer.send('command', 'apache', arg, this.version)
      }
    },
    created: function () {
      if (!this.$store.state.apache.taskRunning) {
        this.$store.dispatch('apache/cleanLog')
      }
    }
  }
</script>
