<template>
  <el-dialog v-model="show" :title="$t('base.customVersionDir')" width="600px" :destroy-on-close="true"
    class="host-edit custom-path" @closed="closedFn">
    <div class="main-wapper">
      <div class="plant-title">
        <span></span>
        <yb-icon :svg="import('@/svg/add.svg?raw')" class="add" width="18" height="18" @click="addDir(undefined)" />
      </div>
      <div class="main">
        <template v-for="(item, index) in dirs" :key="index">
          <div class="path-choose mb-20">
            ...
            <div class="icon-block">
              ...
              <yb-icon :svg="import('@/svg/delete.svg?raw')" class="choose" width="19" height="19"
                @click="delDir(index)" />
            </div>
          </div>
        </template>
