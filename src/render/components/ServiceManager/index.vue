            ...
            <el-popover :show-after="600" placement="top" width="auto">
              ...
              <template #reference>
                <el-button link @click.stop="toPhpMyAdmin">
                  <svg style="width: 29px; height: 29px" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
                    <path
    ...
    <el-table v-loading="service?.fetching" class="service-table" :data="versions">
      <el-table-column prop="version" width="140px">
        ...
        <template #default="scope">
          <span style="padding: 2px 12px 2px 24px; display: block" :class="{
            current:
          ...
          <template v-else>
            <template v-if="scope.row.run">
              <el-button link class="status running" :class="{ disabled: versionRunning }">
                <yb-icon :svg="import('@/svg/stop2.svg?raw')" @click.stop="serviceDo('stop', scope.row)" />
              </el-button>
              <el-button link class="status refresh" :class="{ disabled: versionRunning }">
                <yb-icon :svg="import('@/svg/icon_refresh.svg?raw')" @click.stop="serviceDo('restart', scope.row)" />
              </el-button>
            </template>
            <template v-else>
              <el-button link class="status start" :class="{
                disabled: versionRunning || !scope.row.version,
                current:
                  currentVersion?.version === scope.row.version &&
                  currentVersion?.path === scope.row.path
              }">
                <yb-icon :svg="import('@/svg/play.svg?raw')" @click.stop="serviceDo('start', scope.row)" />
              </el-button>
            </template>
          </template>
        </template>
...
const serviceDo = (flag: 'stop' | 'start' | 'restart' | 'reload', item: SoftInstalled) => {
  ...
  switch (flag) {
    ...
    case 'start':
    case 'restart':
      action = startService(typeFlag, item, currentVersion?.value ? JSON.parse(JSON.stringify(currentVersion.value)) : undefined)
      break
    case 'reload':
