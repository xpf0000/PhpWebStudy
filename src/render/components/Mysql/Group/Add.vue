<template>
  <el-dialog v-model="show" :title="item?.dataDir ? $t('base.edit') : $t('base.add')" width="600px"
    :destroy-on-close="true" class="host-edit new-project" @closed="closedFn">
    <template #default>
      <div class="main-wapper">
        <div class="main">
          <div class="path-choose mt-20 mb-10">
            <el-select v-model="form.path" class="w-p100" :class="{ error: errs?.path }" placeholder="Mysql Version">
          ...
          <div class="path-choose mt-20 mb-20">
            <input v-model.trim="form.port" type="text" class="input" :class="{ error: errs?.port }"
              placeholder="Mysql Port" />
          </div>
          <div class="path-choose mt-20 mb-20">
            <input type="text" class="input" :class="{ error: errs?.dataDir }" placeholder="Mysql Data Dir" readonly=""
              :value="form.dataDir" />
