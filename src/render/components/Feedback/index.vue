<template>
    <el-dialog v-model="show" :title="$t('feedback.sendMessage')" width="600px" :destroy-on-close="true"
        class="host-link-dialog" @closed="closedFn">
        <el-form v-model="FeedbackStore" label-position="left" label-width="90px" @submit.prevent>
            ...
            <el-form-item :label="I18nT('feedback.country')">
                <el-select v-model="FeedbackStore.country" style="width: 100%" filterable clearable
            ...
            <el-form-item :label="I18nT('feedback.message')">
                <el-input v-model.trim="FeedbackStore.message" type="textarea" :minlength="5" :maxlength="1024"
                    :rows="6"></el-input>
            </el-form-item>
        </el-form>
        <template #footer>
            <div class="dialog-footer">
                ...
                <template v-if="FeedbackStore.time === 0">
                    <el-button type="primary" :disabled="FeedbackStore.loading" :loading="FeedbackStore.loading"
                        @click="doSubmit">{{ I18nT('base.confirm') }}</el-button>
                </template>
                <template v-else>
