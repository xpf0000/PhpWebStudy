<template>
  <el-dialog v-model="show" :title="item?.user ? $t('base.edit') : $t('base.add')" width="600px"
    :destroy-on-close="true" class="host-edit new-project" @closed="closedFn">
    <template #default>
      <div class="main-wapper">
        <div class="main">
          <div class="path-choose mt-20 mb-20">
            <input v-model.trim="form.user" type="text" class="input" :readonly="item?.user || running || null"
              :class="{ error: errs?.user }" placeholder="username" />
          </div>
          <div class="path-choose mt-20 mb-20">
            <input v-model.trim="form.pass" type="text" :readonly="running || null" class="input"
              :class="{ error: errs?.pass }" placeholder="password" />
            <div class="icon-block" @click="makePass()">
              <yb-icon :svg="import('@/svg/icon_refresh.svg?raw')" class="choose" width="18" height="18" />
            </div>
          </div>
          <div class="path-choose mt-20 mb-20">
            <input type="text" class="input" :class="{ error: errs?.dir }" placeholder="root path" readonly=""
              :value="form.dir" />
