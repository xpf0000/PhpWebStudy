        ...
        <div class="right">
          <div class="top">
            <el-select v-model="to" filterable @change="onToChange">
              <el-option :label="$t('tools.raw')" value="raw"></el-option>
...
<script lang="ts" setup>
  ...
  import JSONStore, { JSONStoreTab } from './store'
  ...
  const php = computed({
    get() {
      return JSONStore.tabs[JSONStore.currentTab].php?.bin
    },
    set(v: string) {
      const all = brewStore?.php?.installed ?? []
      const find = all.find((p) => p.phpBin === v || join(p.path, 'bin/php') === v)
      JSONStore.tabs[JSONStore.currentTab].php = reactive(
        Object.assign({}, find, {
          bin: v
        })
      )
  ...
  const phpVersion = computed(() => {
    const bin = JSONStore.tabs[JSONStore.currentTab].php?.bin
    const all = brewStore?.php?.installed ?? []
    return all.find((p) => p.phpBin === bin || join(p.path, 'bin/php') === bin)
  })

  const running = computed({
    get() {
      return JSONStore.tabs[JSONStore.currentTab].running
    },
    set(v: boolean) {
      JSONStore.tabs[JSONStore.currentTab].running = v
    }
  })
  ...
  const initFromEditor = () => {
    if (!fromEditor) {
      ...
      fromEditor = EditorCreate(fromRef.value, EditorConfigMake(currentValue.value, 'php'))
  ...
  const initToEditor = () => {
    if (!toEditor) {
      ...
      toEditor = EditorCreate(toRef.value, EditorConfigMake(currentToValue.value, 'javascript'))
