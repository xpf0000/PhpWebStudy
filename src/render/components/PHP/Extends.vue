<template>
  <el-drawer v-model="show" size="80%" :destroy-on-close="true" :with-header="false" :close-on-click-modal="false"
    @closed="closedFn">
    <div class="php-extensions">
      <div class="nav">
        ...
        <el-button type="primary" class="shrink0" :disabled="!installExtensionDir" @click="openDir">{{ $t('base.open')
          }}</el-button>
      </div>
      <div class="main-wapper">
        <el-card>
            ...
            <div class="card-header">
              <div class="left">
                ...
                <template v-if="!extendRunning">
                  <el-select v-model="lib" :disabled="brewRunning || !version?.version" class="lib-select">
                    <template v-if="isHomeBrew">
                      <el-option value="homebrew" label="Homebrew"></el-option>
                    </template>
                    <template v-else>
                      <el-option value="macports" label="Apt"></el-option>
                    </template>
                  </el-select>
                </template>
              </div>
              ...
              <el-button v-else class="button" :disabled="extendRefreshing || extendRunning" link @click="getTableData">
                <yb-icon :svg="import('@/svg/icon_refresh.svg?raw')" class="refresh-icon"
                  :class="{ 'fa-spin': extendRefreshing || extendRunning }"></yb-icon>
              </el-button>
            </div>
          </template>
          ...
          <el-table v-else v-loading="extendRefreshing" height="100%" :data="showTableDataFilter" style="width: 100%">
            ...
            <el-table-column align="center" :label="$t('base.status')">
              <template #default="scope">
                <div class="cell-status">
                  <yb-icon v-if="scope.row.status" :svg="import('@/svg/ok.svg?raw')" class="installed"></yb-icon>
                </div>
              </template>
            </el-table-column>

            <el-table-column width="150px" align="left" :label="$t('base.operation')" class-name="operation">
                  ...
                  <el-popover :show-after="600" placement="top" width="auto">
                    ...
                    <template #reference>
                      <el-button type="primary" link :icon="Link"
                        @click="copyLink(scope.$index, scope.row)"></el-button>
                    </template>
                  </el-popover>
                    ...
                    <el-popover :show-after="600" placement="top" width="auto">
                      ...
                      <template #reference>
                        <el-button type="primary" link :icon="Document"
                          @click="copyXDebugTmpl(scope.$index, scope.row)"></el-button>
                      </template>
                    </el-popover>
                  </template>
                  <el-popover :show-after="600" placement="top" width="auto">
                    ...
                    <template #reference>
                      <el-button type="primary" link :icon="Delete" @click="doDel(scope.$index, scope.row)"></el-button>
                    </template>
                  </el-popover>
                </template>
                <template v-else>
                  <el-popover :show-after="600" placement="top" width="auto">
                    ...
                    <template #reference>
                      <el-button :disabled="brewRunning || !version?.version" type="primary" link :icon="Download"
...
const fetchAll = (fn: 'fetchAllPhpExtensions' | 'fetchAllPhpExtensionsByPort') => {
  return new Promise((resolve) => {
    ...
    IPC.send('app-fork:brew', fn, JSON.parse(JSON.stringify(props.version))).then((key: string, res: any) => {
      IPC.off(key)
      if (res?.data) {
